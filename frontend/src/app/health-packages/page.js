...
export default function HealthPackagesPage() {
        ...
        setPackages([
          {
            ...
            price: "Rs. 2,500",
            features: ["Complete Blood Count", "Blood Pressure Check", "Basic Metabolic Panel", "General Physician Consultation"]
          },
          {
            _id: 2,
            ...
            price: "Rs. 8,500",
            features: ["ECG Test", "Lipid Profile", "Cardiac Enzymes", "Cardiologist Consultation"]
          },
          {
            _id: 3,
            ...
            price: "Rs. 5,200",
            features: ["Blood Sugar Profile", "HbA1c Test", "Dietitian Consultation", "Endocrinologist Review"]
          },
          {
            _id: 4,
            ...
            price: "Rs. 12,000",
            features: ["Complete Health Checkup", "Bone Density Scan", "Vision & Hearing Tests", "Geriatrician Consultation"]
          },
          {
            _id: 5,
            ...
            price: "Rs. 9,800",
            features: ["Mammography", "Pap Smear", "Hormone Profile", "Gynecologist Consultation"]
          },
          {
            _id: 6,
            ...
            price: "Rs. 25,000",
            features: ["Full Body MRI", "Advanced Cardiac Screening", "Cancer Markers", "Specialist Consultations"]
          }
        ]);
      } finally {
        setLoading(false);
  ...
  return (
    <>
      <style>{`
        ...
        .hp-card { background-color: white; border-radius: 15px; padding: 2rem; box-shadow: 0 10px 25px rgba(0,0,0,0.1); border: 1px solid #e2e8f0; transition: transform 0.3s ease, box-shadow 0.3s ease; position: relative; overflow: hidden; box-sizing: border-box; }
      ...
      <div className="hp-outer">
        <div className="hp-container">
          <div className="hp-header">
            ...
            <p>Choose from our preventive health packages designed to keep you and your family healthy.</p>
          </div>
          ...
          <div className="hp-grid">
            {packages.map((pkg, index) => (
              <article key={pkg._id || index} className="hp-card">
                <div
                  className="hp-badge"
                  style={{ backgroundColor: index === 0 ? '#10b981' : index === 1 ? '#f59e0b' : '#8b5cf6' }}
                >
                  Popular
                </div>
        ...
        {showBookingModal && selectedPackage && (
          <div className="hp-modal-overlay" onClick={() => setShowBookingModal(false)}>
            <div className="hp-modal" onClick={(e) => e.stopPropagation()}>
              <div className="hp-modal-header">
                <h3>Book {selectedPackage.name}</h3>
                <button className="hp-modal-close" onClick={() => setShowBookingModal(false)}>×</button>
              </div>

              <div className="hp-modal-details">
                <p style={{ fontWeight: '600', color: '#1e40af' }}>Package Details:</p>
                <p style={{ color: '#4b5563' }}>{selectedPackage.description}</p>
                <p style={{ fontWeight: 'bold', color: '#0b7ac4', fontSize: '1.2rem' }}>{selectedPackage.price}</p>
              </div>
              ...
              <form
                ...
                onSubmit={async (e) => {
                  ...
                  try {
                    ...
                    alert(`Thank you! Booking request for ${selectedPackage.name} received.`);
  ...
  */
        ...
        />
                ...
                <button type="submit" className="hp-confirm-btn">Confirm Booking</button>
              </form>
            </div>
          </div>
        )}
